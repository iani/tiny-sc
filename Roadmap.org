...
\lpf =< ({ GrayNoise.ar } ==> \source);

// No problem changing source like this:
{ WhiteNoise.ar } ==> \source;
// But re-inputtig issues /n_set Node not found (but sound is ok):
\lpf =< ({ SinOsc.ar(400) } ==> \source);

/// More stuff ...

{ In.ar(\in.kr(0)) * Decay.kr(Dust.kr(4)) } =<> \lpf;
\lpf =< ({ LFTri.ar(LFNoise2.kr(7.15).range(400, 4000)) } ==> \source2);


{ LFPulse.ar(LFNoise0.kr(10.1).range(300, 400)) } ==> \source2;
{ LFTri.ar(LFNoise2.kr(7.15).range(400, 4000)) } ==> \source2;
...
\mySynth
   ...
   .out(param, chans)
   .in(param, chans)
   .map(name, param, chans)
   .buf(name, param, chans)
   .add(element, param, path);
   .sub(element, param, path);
   .mul(element, param, path);
   .div(element, param, path);
   .mod(element, param, path);
   .pow(element, param, path);
   .sel(element, param, path);
   .rej(element, param, path);
   .fun(element, param, path);
   .choose(element, param, path);
   .wchoose(element, param, path);
#+END_EXAMPLE
