...
*** Object:src : Make any object receive messages from a data source

: anObject.src(source);

Get or create a Source instance, and connect its output to anObject.  Return the Source instance.  The way in which the output is connected to the receiving object is set by invoking a method of the Source instance through a message.  For example:

: anObject.src(source).set(\freq, [500, 600].asSpec);

The above makes the object =source= send to =anObject= (usually a Synth instance), the message =set= for setting parameter =freq= with a value mapped from an input in the range of 0-1 to the range of 500-600.  The source can be any object that generates values in time, for example a Routine, an OSCFunc, a MIDIFunc, a gui Slider, etc.  The object given to method =src= as its =source= parameter can be either a symbol for accessing an already existing Source instance from a global dictionary, or a template that is used by Source to create a Source instance which will broadcast the generated values.
...
1. Find or create the source instance.
   ...
   - Otherwise create an instance of source using the object as source of values.  The object given in =source= is treated differently according to its kind:
     ...
     - function: create a Stream from =Pfunc(function)=
     - number: create a Stream from =Pn(number, inf)=
     - view: set the action of the view to send its value to the source for broadcasting.
     - OSCFunc: set the func of ...
     - MIDIFunc: set the func of ...
     - Array: Pseq(array, inf).asStream, polled at intervals given by Source.pollRate.





**** Shortcut: set parameter of Node from values
 A special case/shortcut method for setting the parameter of a Node (Synth or Group):
: aNode.functl(routname[->parname], valueStream, timeStream)
...
*** Source (Broadcast?): encapsulate routine or other data source in an object
