...
bus should be freed and removed from global dictionary when all of its synth inputs are freed.

*** rmap

rmap(parameter, valueStream, timeStream, controlName)
...
- =controlName= :: A symbol.  Defaults to the value of =parameter=.  rmap will try to access an existing routine
A routine is accessed by \controlName, or created:
