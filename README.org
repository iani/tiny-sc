...
** =Pub= (currently still defined as =Source=) Flexibly connect objects to data sources

Make any object receive messages from a data source.  Set or replace the origin of the data source at any time. Work interchangeably with Patterns/Streams, Views, OSCFuncs, MIDIFuncs, constant values.  Customize, map and/or filter the messages and values sent to listening objects.

Note: Currently (Wed, Mar  5 2014, 17:08 EET) this class is called =Source=.  But this creates confusion, because the Source actually publishes to many objects the results of polling another object, which is the stream *source*.  So it would be better to call this class "Publisher", or for short: "Pub".

*** Basic method: =src= (or =pub= ?)

: anObject.src(source, mapper);
...
: anObject.src(source, [500, 600].mapSet(\freq));
...
** PatternPlayer: Play a pattern, as stream, in time
...
Used by Source as default source for all objects except OSCFunc, MIDIFunc and View.
