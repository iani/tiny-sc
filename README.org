...
Note: As of Tue, Jun 16 2015, 23:08 EEST a new audio-linking mechanism is under development here, inspired by Steno of Julian Rohrhuber.  The old linking mechanism is broken, but the rest of the library is intact.
...
One can use these separately, or one may combine them in a single expression.  The first example shows a combined use:

#+BEGIN_SRC sclang
{ SinOsc.arps(Rand(400, 1000)) } +> \test0 *> 0.1;
#+END_SRC
...
#+BEGIN_SRC sclang
{ SinOsc.arps(Rand(1000, 2000)).perc } ++>.2 \test0;
#+END_SRC
...
The safe speed limits on my MacBook Pro are: 0.01 duration for events with self-releasing envelope and 0.02 duration for those without self-releaseing envelope  (see =perc=, =sine=). but they depend on the load of other processes.  At 500 events per second, the scheduler may get stuck occasionally when switching from one pattern to another. The following example with 0.005 duration may be risky on your computer:
...
** COMMENT Linking audio inputs and outputs between synths
:PROPERTIES:
:ID:       EA903FD5-9FE9-4B0A-BEE0-B38C8807810E
:eval-id:  617
:END:

*THIS IS IN PROCESS OF BEING REPLACED BY A STENO-TYPE BASED MECHANISM*

Following examples do not work.

The audio output of a Chuck can be linked to the input of another Chuck.  Many-to-one and one-to-many audio link configurations are possible.  The library deals with bus allocation and synth order management in the background.

Start a source and an effect:
#+BEGIN_SRC sclang
{ PinkNoise.ar(0.1) } ++> \source;  // this will be heard without effect - not linked yet!
{ Resonz.arp(In.ar(\in.ar(0)) * 350, LFNoise0.kr(30).range(400, 2000), 0.001) } ++> \effect;
#+END_SRC
...
#+BEGIN_SRC sclang
\source.toRoot;  // now is "unlink" - need to provide both unlink and toRoot
#+END_SRC

Add a second effect:

#+BEGIN_SRC sclang
{ In.ar(\in.kr(0)).abs * 7 * Decay.kr(Dust.kr([1.5, 1.5]), 1.5) } ++> \effect2;
#+END_SRC
...
NEEDS CORRECTION! Remove source from effect2
#+BEGIN_SRC sclang
\source !> \effect2; // also removes from effect!
#+END_SRC
