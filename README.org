...
With duration limited by the envelope (=perc=), playing this at 100 events per second is safe.  At 500 events per second, the scheduler may get stuck occasionally when switching from one pattern to another - so the above example with 0.005 duration was already risky!
