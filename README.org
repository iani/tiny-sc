...
// So, one can chuck into them:
0.03 => ~amp;
// Change channels again
{ 30 do: { 2.rand => ~out; 0.1.wait } }.fork;
// Set a sound with frequency to play with:
{ LFSaw.ar(\freq.kr(440)) } => \sound;
// Play with frequency
{ 150 do: { 50.rrand(80).midicps => ~freq; 0.05.wait }; }.fork;
