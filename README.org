...
Otherwise use the *> operator to set the \instr parameter (synonym: \instrument):
#+BEGIN_EXAMPLE
[\sine, \werkitd].prand *> \instr;
#+END_EXAMPLE

Setting legato:

#+BEGIN_EXAMPLE
{ 0.1 exprand: 1.5 }.pfunc *> \legato;
#+END_EXAMPLE

To set the instrument parameter to a named tree, use -> to associate the patttern with the parameter:
#+BEGIN_EXAMPLE
[\sine, \werkitd, \spacetheremin].prand -> \instr => \test2;
#+END_EXAMPLE

Setting other parameters of the synthree pattern merges these into the existing instrument pattern, leaving the instrument, duration or other parameters as they were:
#+BEGIN_EXAMPLE
[freq: [400, 500].prand, amp: { 0.01 exprand: 0.1 }.pfunc] => \test2;
#+END_EXAMPLE

*** Note: SynthDef as Instrument

Chucking an array of parameters to a SynthTree that is not playing an InstrumentPattern will create an instrument pattern, keeping the existing instrument, if the template is a SynthDef:

#+BEGIN_EXAMPLE
SynthDef("saw", { LFSaw.ar(\freq.kr(400)).adsrOut }).add => \instrumentTest;
#+END_EXAMPLE

#+BEGIN_EXAMPLE
[freq: [400, 500].prand, amp: { 0.01 exprand: 0.1 }.pfunc] => \instrumentTest;
#+END_EXAMPLE
