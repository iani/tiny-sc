...
Facilities for modifying the tree structure created by compiling a Steno string are under preparation.  These will allow to add Chucks with longer names to the tree, and to link Chucks without running a Steno string.
