...
Stop all synths by typing Command-. (on SC IDE), or Control-c Control-s (on Emacs), or by evalyating this:
...
Stop or restart the example above by typing =H-c H-space=.
