...
- \edef => \synthtree :: play new Bdef from edef into synthree.
  - Question: hat if the same edef's bdef is already playing in this synthtree?  Start a new Bdef, or do nothing?
- (event) => \synthtree :: play event as Bdef in synthree / or modify existing Bdef
- \edef =< (event) :: Create new edef if needed.  If the Edef has no children, then create a new Idef child and play it.  Else modify existing edef.
- (event) +> \edef :: Is this operator now obsolete given the meaning of =< above?
