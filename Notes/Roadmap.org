...
*** option 1: store under name in dictionary inside parameter
...
*** option 2: store sources in set in param - not in dict (no names)

*** option 3: store sources only globally in Library - not in param at all
