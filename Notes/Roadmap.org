...
- array => \symbol[\patname] :: play array as event in parameters of synthtree, naming the pattern, so that multiple patterns can be played.
- \sourcename => \symbol :: play named source into synthree, in manner depending on type of source.  Adverb and index specifications apply.  Use shortcut methods to specify type of sourcename to be accessed:
  - b :: buffer
  - p :: pattern, single, cloned from Edef
  - pm :: pattern, shared from Bdef
  - f :: function (for kr synth)
  - t :: template (synthdef)
  - o :: osc
  - m :: midi
  - v :: view
