...
** Designing Edef, Cdef, Istream, Bstream
:PROPERTIES:
...
** New README opening examples, starting with Mdef.
:PROPERTIES:
:DATE:     <2014-04-22 Tue 15:26>
:END:

See file MdefTests.scd
** Use embed in stream to chain (=embed) PatternPlayers?
:PROPERTIES:
:DATE:     <2014-04-20 Sun 18:32>
:END:

Embedding of event-patterns done.  See EventPattern.

Next:

Make Edef and SynthTree play with EventPattern instead of PatternEventPlayer.

Note: All inheritance/modifications should be done with Edef, its related classes Cdef (inheriting clone of Edef), Istream (inheriting named wrapper around EventStreamPlayer), Bstream (subclass of Istream that makes the streamplayer broadcast instead of playing) or SynthTree.  Keep EventPattern, EventStream as simple as possible.
...
And in Event, add =broadcastAndDelta=, keeping all all code, and changing only the line =this.play= to streamPlayer.changed(this), so that receivers may optionally play a modified version of the event, or otherwise react in different ways.
...
** Make PatternPlayer/Instrument fully compatible to Event

incorporate legato, dur, and instrument in PatternPlayer's pattern.
