...
This review may result in some operators (eg *>) becoming obsolete.

Some of the things described here are yet to implement.
...
: (event) -> duration pattern => sourcetree
or
: [array] -> duration pattern => sourcetree

Play all key-pattern pairs in event or array in corresponding parameters of sourcetree, with common duration pattern.
...
: value pattern -> duration pattern =>.parameter sourcetree

Play value pattern with duration pattern in parameter of sourcetree.

*** source +> something: Modify source of synthtree

Modifies stream that plays in synthtree.

*** source %> something: Add filter to source of synthtree

Adds filter to stream that plays in synthree.

*** source *> something: Obsolete? (!)

Covered by variants of => using associations (see above).
...
Or to play a pattern in a parameter!  Note:  One could play these as EvtStreams (even Edefs or Idefs), using a set eventType, that does synth.set(param, val) as its action.  See SC Help, =Event types=:
...
- (event) => `\idef :: Play event as source of idef's EvtStream.  Create Idef if it does not exist.  In any case, idef becomes parentless (!).
  - STATUS: TODO.
...
(Renamed from EventList to avoid clash with FP quark)
...
:END:
(Note to self: Forget about nesting here.  For that you need EventSequence.)
...
play { arg clock, protoEvent, quant;
		^this.asEvtStreamPlayer(protoEvent).play(clock, false, quant)
	}

	asStream { ^Routine({ arg inval; this.embedInStream(inval) }) }
	iter { ^this.asStream }

	asEvtStreamPlayer { arg protoEvent;
		^EvtStreamPlayer(this.asStream, protoEvent);
	}
...
#+BEGIN_EXAMPLE
	...
	asEvtStreamPlayer { arg protoEvent;
		^EvtStreamPlayer(this, protoEvent);
	}
...
And also in EvtStreamPlayer this:
...
See class EventPattern, EvtStream.
...
Embedding of event-patterns done.  See EventPattern, EvtStream.
...
Try making Edef subclass of EventPattern and Idef subclass of EvtStream.  The reason is to implement the alternative asStream and embedInStream methods without having to add exra wrappers in a different class to handle them.
...
A mod takes the event inherited from the parent and applies modifications to it in order to generate a modified Event that is used by the EventPattern or EvtStream.
...
Note: All inheritance/modifications should be done with Edef, its related classes Cdef (inheriting clone of Edef), Idef (inheriting named wrapper around EvtStreamPlayer), Bdef (subclass of Istream that makes the streamplayer broadcast instead of playing) or SynthTree.  Keep EventPattern, EvtStream as simple as possible.
...
- The EvtStreamPlayer that it is playing (so that it can start it or stop it).
...
Use a subclass of EvtStreamPlayer, called EvtStreamBroadcaster that uses a variant of prPlay as follows:
...
**** Runtime modifications apply to EvtStreamBroadcasters, not Mdefs (!?)
...
#+BEGIN_EXAMPLE
Mdef(\x).play; // plays into EvtStreamBroadcaster (Bdef) of same name
Mdef(\x).play(\y) // plays into EvtStreamBroadcaster named \y
...
Not applicable, because the values received from the playing EvtStream are final ones - the result of playing the Stream.  It is however possible to use a preexisting stream to compute new values and combine them with the value from the event.
...
To kill all child-processes of a Routine one may add thisThread as notifier to a child process (Node or Routine or EvtStreamPlayer) and notify the children when the thread stops.  Similarly for EvtStreamPlayer.  Registering processes under a snippet/org-section id is a simpler alternative. However process-tree based stopping is a different thing, because one may want to kill a parent-process through the gui, independent fom the snippet grouping which may contain also other processe
